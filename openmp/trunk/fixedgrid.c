...
void discretize_all_rows(fixedgrid_t* G, double dt)
{
    ...
    #pragma omp for private(i, j, s, buff, cbound, wbound, dbound)
    for(i=0; i<NROWS; i++)
    {
        #ifdef DO_CHEMISTRY
        for(s=0; s<NSPEC; s++)
...
void discretize_all_cols(fixedgrid_t* G, double dt)
{
    ...
    #pragma omp for private(j, k, s, ccol1, ccol2, wcol, dcol, cbound, wbound, dbound)
    for(j=0; j<NCOLS; j++)
    {
        #ifdef DO_CHEMISTRY
        for(s=0; s<NSPEC; s++)
...
int main(int argc, char** argv)
{
    ...
    /* Factor out duplicate metrics */
    G->metrics.array_copy.elapsed /= (G->nprocs);
    G->metrics.row_discret.elapsed /= (G->nprocs);
    G->metrics.col_discret.elapsed /= (G->nprocs);
    G->metrics.tot_discret.elapsed /= (G->nprocs);
    G->metrics.chem.elapsed /= (G->nprocs);
    
    /* Print metrics */
    print_metrics(&G->metrics);
    
    /* Write metrics to CSV file */
    write_metrics_as_csv(G, "OpenMP");
