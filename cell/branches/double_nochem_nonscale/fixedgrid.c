...
double concBuff[NROWS*SPE_MAX_THREADS] __attribute__((aligned(128)));
double windBuff[NROWS*SPE_MAX_THREADS] __attribute__((aligned(128)));
double diffBuff[NROWS*SPE_MAX_THREADS] __attribute__((aligned(128)));
double outBuff[NROWS*SPE_MAX_THREADS] __attribute__((aligned(128)));

/**
 * Process rows 1/2 timestep 
 */
void discretize_row(fixedgrid_t* G, uint32_t spec, double dt)
{
 ...
 */
void discretize_col(fixedgrid_t* G, uint32_t spec, double dt)
{
    uint32_t i, j;
    ...
    for(j=0; j<NCOLS; j++)
    {
        ...
        timer_start(&G->ppe_metrics.array_copy);
        ...
        if(j >= G->nprocs)
        {
            for(i=0; i<NROWS; i++)
            {
                G->conc(spec, i, j - G->nprocs) = concBuff[proc*NROWS+i];
            }
        }
        
        for(i=0; i<NROWS; i++)
        {
            concBuff[proc*NROWS+i] = G->conc(spec, i, j);
            windBuff[proc*NROWS+i] = G->wind_v[i*NCOLS+j];
            diffBuff[proc*NROWS+i] = G->diff[i*NCOLS+j];
        }
        
        timer_stop(&G->ppe_metrics.array_copy);
        
        timer_start(&G->ppe_metrics.comm);
        
        /* Configure SPE arguments */
        G->threads[i].argv.arg[0].u64 = (uint64_t)(&concBuff[proc*NROWS]);
        G->threads[i].argv.arg[1].u64 = (uint64_t)(&windBuff[proc*NROWS]);
        G->threads[i].argv.arg[2].u64 = (uint64_t)(&diffBuff[proc*NROWS]);
        G->threads[i].argv.arg[3].dbl = dt;
        G->threads[i].argv.arg[4].dbl = DY;
        G->threads[i].argv.arg[5].u32[0] = NROWS;
        G->threads[i].argv.arg[5].u32[1] = 1;
        G->threads[i].argv.arg[6].dbl = G->temp;
        G->threads[i].argv.arg[7].dbl = 0.0;
        
        /* Signal SPE */
        spe_set_status(G, proc, SPE_STATUS_ROW);
