...
void init_model(uint64_t argvp)
{
    ...
    TIME = (real_t)argv.arg[7].dbl;
    DT   = (real_t)argv.arg[9].dbl;
    SUN  = (real_t)0.0; // set at integration
    TEMP = (real_t)0.0; // FIXME: set at integration
    STEPMIN = 0.01;
...
int main(uint64_t id __attribute__((unused)), uint64_t argvp, uint64_t envvp)
{
    ...
    while(status.value != SPE_STATUS_STOPPED)
    {
        ...
        {
            case SPE_STATUS_X:
                discretize_x_block(argvp);
                break;
                
            case SPE_STATUS_Y:
                discretize_y_block(argvp);
                break;
                
            case SPE_STATUS_Z:
                discretize_z_block(argvp);
                break;
