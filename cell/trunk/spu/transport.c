...
/* Shuffle buffers for x discretization */
...
/**
 * Fetches y data from main memory
 */
void fetch_y_buffer(uint32_t i, uint64_t ea_off)
{
    uint32_t j, x;
    ...
    clist[i].data[j].eal = conc[i].ea_base + ea_off + j*NX_ALIGNED_SIZE; \
    wlist[i].data[j].eal = wind[i].ea_base + ea_off + j*NX_ALIGNED_SIZE; \
    dlist[i].data[j].eal = diff[i].ea_base + ea_off + j*NX_ALIGNED_SIZE; \
...
/**
 * Writes x data to main memory
 */
void write_x_buffer(uint32_t i, uint64_t ea_off)
{
 ...
 */
void write_y_buffer(uint32_t i)
{
