...
void allocate_global_memory(fixedgrid_t* G)
{
    ...
    //G->conc = (real_t*)_malloc_align(ROW_LENGTH*NCOLS*NSPEC*sizeof(real_t), 7);
    
    /* Allocate wind vector filed memory */
    //G->wind_u = (real_t*)malloc_align(NROWS*NCOLS*sizeof(real_t), 7);
    //G->wind_v = (real_t*)malloc_align(NROWS*NCOLS*sizeof(real_t), 7);
    
    /* Allocate diffusion tensor memory */
    //G->diff = (real_t*)malloc_align(NROWS*NCOLS*sizeof(real_t), 7);    
}
