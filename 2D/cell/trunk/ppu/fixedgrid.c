...
void start_discretize_row(fixedgrid_t* G, uint32_t spec, real_t dt)
{
    ...
    for(i=0; i<G->nprocs; i++)
    {
        ...
        G->threads[i].argv.arg[6].dbl = 0.0;
...
void start_discretize_col(fixedgrid_t* G, uint32_t spec, real_t dt)
{
    ...
    block = (ROW_LENGTH / VECTOR_LENGTH) / G->nprocs;
    
    for(i=0; i<G->nprocs; i++)
    {
        /* Configure SPE arguments */
        G->threads[i].argv.arg[0].u64 = (uint32_t)(&G->conc(spec, 0, i*block*VECTOR_LENGTH));
        G->threads[i].argv.arg[1].u64 = (uint32_t)(&G->wind_v(0, i*block*VECTOR_LENGTH));
        G->threads[i].argv.arg[2].u64 = (uint32_t)(&G->diff(0, i*block*VECTOR_LENGTH));
        G->threads[i].argv.arg[3].dbl = dt;
        G->threads[i].argv.arg[4].dbl = DY;
        G->threads[i].argv.arg[5].u32[0] = NROWS;
        G->threads[i].argv.arg[5].u32[1] = (i == G->nprocs - 1 ? (ROW_LENGTH/VECTOR_LENGTH) - (G->nprocs-1)*block : block);  //FIXME: Potential load imbalance
        G->threads[i].argv.arg[6].dbl = 0.0;
        G->threads[i].argv.arg[7].dbl = 0.0;
