...
/*~~~> Function headers */   
 void FunTemplate(double, volatile double [], double []); 
 void JacTemplate(double, volatile double [], double []) ;
 int Rosenbrock(volatile double Y[], double Tstart, double Tend,
     volatile double AbsTol[], volatile double RelTol[],
     double RPAR[], int IPAR[]);
 int RosenbrockIntegrator(
     volatile double Y[], double Tstart, double Tend ,     
     volatile double  AbsTol[], volatile double  RelTol[],
     int ros_S,
 ...
 void ros_FunTimeDerivative ( 
     ...
     volatile double Y[], double Fcn0[], 
     double dFdT[] );
 void Fun( volatile double Y[], volatile double FIX[], volatile double RCONST[], double Ydot[] );
 void Jac_SP( volatile double Y[], volatile double FIX[], volatile double RCONST[], double Ydot[] );
 void FunTemplate( double T, volatile double Y[], double Ydot[] );
 void JacTemplate( double T, volatile double Y[], double Ydot[] );
 void DecompTemplate( double A[], int Pivot[], int* ising );
 void SolveTemplate( double A[], int Pivot[], double b[] );
 void WCOPY(int N, volatile double X[], int incX, volatile double Y[], int incY);
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
{   
   double Ynew[74], Fcn0[74], Fcn[74],
      dFdT[74],
      Jac0[920], Ghimj[920];
   double K[74*ros_S];   
   double H, T, Hnew, HC, HG, Fac, Tau; 
   double Err, Yerr[74];
   int Pivot[74], Direction, ioffset, j, istage;
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
  ...
  if (!Autonomous) {
      ros_FunTimeDerivative ( T, Roundoff, Y, Fcn0, dFdT );
  }
  ...
  /*~~~>   Compute the stages  */
   ...
   {
      /* Current istage offset. Current istage vector is K[ioffset:ioffset+74-1] */
      ioffset = 74*(istage-1);
       
      /* For the 1st istage the function has been computed previously */
      if ( istage == 1 )
      {
          WCOPY(74,Fcn0,1,Fcn,1);
      }
      ...
      { 
          if ( ros_NewF[istage-1] )
          {
              ...
              {
                  WAXPY(74,ros_A[(istage-1)*(istage-2)/2+j-1], &K[74*(j-1)],1,Ynew,1);
              }
  ...
  /*~~~>  Compute the new solution   */
   WCOPY(74,Y,1,Ynew,1);
   for (j=1; j<=ros_S; j++)
       WAXPY(74,ros_M[j-1],&K[74*(j-1)],1,Ynew,1);

  /*~~~>  Compute the error estimation   */
   WSCAL(74,ZERO,Yerr,1);
   for (j=1; j<=ros_S; j++)    
       WAXPY(74,ros_E[j-1],&K[74*(j-1)],1,Yerr,1);
...
{   
  ...
  /*~~~> Note: for a full matrix use Lapack:
      NRHS = 1
      DGETRS( 'N', 74 , NRHS, A, 74, Pivot, b, 74, INFO ) */
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/   
void JacTemplate( double T, volatile double Y[], double Jcb[] )
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   
